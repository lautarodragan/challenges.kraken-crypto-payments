...
1. For the minimum valid deposit I've ignored transactions with an amount of zero. There happens to be only one such transaction: `d1579f9602d3a6c783daac95fcea3390f57adff85455233de000f90c128c4cfe`. Transactions that show up with an amount of zero in `listsinceblock` can be transactions with scripts that aren't "moving" bitcoins from one address to the other, such as colored coins transactions. 
1. The smallest non-zero deposit is `d1579f9602d3a6c783daac95fcea3390f57adff85455233de000f90c128c4cfe` with an amount of `1e-7`. I believe (but am not sure) bitcoind will never return an amount of bitcoins in exponential notation, the RPC calls generally use [ValueFromAmount](https://github.com/bitcoin/bitcoin/blob/a28ea316ed47d4e3e30de3fbd77d010f7f9c24ea/src/test/rpc_tests.cpp#L140-L173) to avoid such notation and even ensure padding. Feeding the output of `listsinceblock` to `jq` or other programs _will_ parse the number and re-render it in `e` notation, possibly losing precision, which I suspect is what happened in the provided file.
1. The smallest non-zero deposit, `1e-7`, is possibly lower than the transaction fee, meaning Kraken could lose money accepting it. The Kraken UI (as of Jan 2020) states the minimum deposit is of ` ₿0.0001`, which is more reasonable. Hence I decided to consider deposits less than `0.0001` invalid.
1. Amounts are rendered with decimal.js to avoid exponential notation and to ensure trailing zeroes are added as needed.
